...
const SideBar = () => {
  ...
  const getNavLinkClass = (isActive) => {
    const base = "group flex items-center gap-4 px-6 py-4 rounded-2xl transition-all duration-300 mb-2 relative overflow-hidden";
    ...
    <aside className="fixed left-0 top-0 h-screen w-72 bg-[#002b5c] flex flex-col z-50 shadow-[10px_0_40px_rgba(0,0,0,0.1)] border-r border-white/5 font-sans">
      <div className="p-10 shrink-0">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-[10px] font-black text-white/40 uppercase tracking-[0.4em]">Admin Portal</span>
        </div>
      ...
      <nav className="flex-1 px-6 overflow-y-auto custom-scrollbar pt-4">
        <div className="mb-6">
          <p className="px-6 text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-4">Main Menu</p>
          {menuItems.map((item) => (
            <NavLink 
              key={item.label} 
              to={item.route}
            ...
            >
              {({ isActive }) => (
                <>
                  <item.icon size={22} className={isActive ? "text-[#002b5c]" : "text-white/40 group-hover:text-white"} />
      ...
      <div className="p-8 border-t border-white/5 bg-black/10">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="overflow-hidden">
            ...
            <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Master Access</p>
          </div>
        </div>
